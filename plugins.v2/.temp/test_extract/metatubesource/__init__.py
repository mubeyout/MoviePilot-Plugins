...
class MetatubeSource(_PluginBase):
    ...
    _bytemuse_api_url: str = "http://127.0.0.1:3750"  # ByteMuse API 地址
    _bytemuse_api_token: str = ""  # ByteMuse API Token
    ...
    def init_plugin(self, config: dict = None):
        ...
        if config:
            ...
            self._bytemuse_api_url = config.get("bytemuse_api_url") or "http://127.0.0.1:3750"
            self._bytemuse_api_token = config.get("bytemuse_api_token") or ""
            # 新增配置项
        ...
        self._bytemuse_client = ByteMuseApiClient(
            base_url=self._bytemuse_api_url,
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        if self._metatube_client and self._metatube_client.test_connection():
            results.append("Metatube: 连接成功")

        # 测试 ThePornDB 连接
        if self._theporndb_enabled and self._theporndb_api_token:
            if self._theporndb_client and self._theporndb_client.test_connection():
                results.append("ThePornDB: 连接成功")

        # 测试 ByteMuse 连接
        if self._bytemuse_enabled and self._bytemuse_api_token:
            if self._bytemuse_client and self._bytemuse_client.test_connection():
                results.append("ByteMuse: 连接成功")
    ...
    def get_form(self) -> Tuple[List[dict], Dict[str, Any]]:
        ...
        return [
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "label": "启用ThePornDB",
                                            "hint": "补充识别方案（当Metatube失败时使用，支持日本番号和欧美内容）"
                                        }
                                    }
                                ]
                            },
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "label": "启用ByteMuse",
                                            "hint": "备用识别方案（当Metatube和ThePornDB都失败时使用）"
                                        }
                                    }
                                ]
                            },
                                ...
                                "content": [
                                    {
                                        ...
                                        "props": {
                                            ...
                                            "label": "ByteMuse API Token",
                                            "placeholder": "可选，如果ByteMuse启用了认证",
                                            "hint": "ByteMuse API Token（可选）"
                                        }
                                    }
                                ]
                            }
            ...
            "bytemuse_api_url": "http://127.0.0.1:3750",
            "bytemuse_api_token": ""
        }
    ...
    def _is_jav_number(self, number: str) -> bool:
        ...
        # 日本番号常见前缀
        jav_prefixes = [
            'SONE-', 'SSIS-', 'SSND-', 'SNIS-', 'IPX-', 'IPZ-', 'IPZZ-',
            'MIDE-', 'MIDV-', 'MIPX-', 'MIAE-', 'MIRD-', 'EBOD-',
            'WANZ-', 'WAAA-', 'ABP-', 'ABW-', 'BKSP-', 'FC2-',
            'HEYZO-', 'CARIB', '1PONDO-', 'SKY-', 'RED-',
            'H0930-', 'C0930-', 'KIN8-', 'GOLD-', 'CWP-',
            'VR-', '3DVR-', 'MDTV-', 'MDX-',
            '051', '062', # 数字前缀系列
        ]
        ...
        # 匹配纯字母-数字格式的番号 (如 ABC-123)
        import re
        jav_pattern = r'^[A-Z]{2,6}-?\d{3,5}$'
        if re.match(jav_pattern, number_upper):
            return True
    ...
    def _convert_theporndb_jav_to_mediainfo(self, jav: ThePornDBJAV,
                                           detail: Optional[ThePornDBJAVDetail] = None) -> MediaInfo:
        """将 ThePornDB JAV 结果转换为 MediaInfo"""
        mediainfo = MediaInfo()
        mediainfo.source = 'theporndb_jav'
        mediainfo.type = MediaType.MOVIE  # 作为电影处理

        # 基础信息 - 使用番号作为标题的一部分
        if jav.external_id:
            # 如果有番号，使用 "番号 - 标题" 格式
            mediainfo.title = f"{jav.external_id} - {jav.title}" if jav.title else jav.external_id
            mediainfo.original_title = jav.title or jav.external_id
        else:
            mediainfo.title = jav.title
            mediainfo.original_title = jav.title

        # 解析日期获取年份
        if jav.date:
            try:
                date_str = jav.date.split('T')[0] if 'T' in jav.date else jav.date
                mediainfo.year = date_str[:4]
                mediainfo.release_date = date_str
            except Exception:
                pass
        ...
        # 使用番号作为标识
        mediainfo.imdb_id = jav.external_id or jav.id

        # 海报和背景
        if jav.background and jav.background.large:
            mediainfo.poster_path = jav.background.large
        elif jav.poster:
            mediainfo.poster_path = jav.poster
        if jav.background and jav.background.full:
            mediainfo.backdrop_path = jav.background.full

        # 时长
        if jav.duration:
            mediainfo.runtime = jav.duration // 60  # 秒转分钟
        ...
        if detail:
            ...
            if detail.directors:
                # 处理导演信息
                directors = [d.get('name', d) for d in detail.directors if d.get('name')]
                if directors:
                    mediainfo.directors = directors

        # 如果详情中没有演员，使用搜索结果的演员
        if not mediainfo.actors and jav.performers:
            mediainfo.actors = [{"name": p.name} for p in jav.performers]

        # 站点信息作为制作商
        if jav.site and jav.site.name:
            mediainfo.studio = jav.site.name

        # JAV 分类
        category = "成人/JAV"
        mediainfo.set_category(category)
        logger.info(f"ThePornDB JAV: 分类设置为 '{category}' (番号: {jav.external_id}, 标题: {jav.title})")

        return mediainfo

    def _convert_bytemuse_to_mediainfo(self, video: ByteMuseMovie) -> MediaInfo:
        ...
        # 基础信息
        mediainfo.title = video.title or video.code
        mediainfo.original_title = video.title or video.code

        # 解析日期获取年份
        if video.release_date:
            try:
                date_str = video.release_date.split('T')[0] if 'T' in video.release_date else video.release_date
                mediainfo.year = date_str[:4]
                mediainfo.release_date = date_str
            except Exception:
                pass
        ...
        # 使用番号作为标识
        mediainfo.imdb_id = video.code

        # 海报和背景
        if video.poster:
            mediainfo.poster_path = video.poster
        if video.banner:
            mediainfo.backdrop_path = video.banner

        # 时长
        if video.duration:
            mediainfo.runtime = video.duration

        # 演员
        if video.casts:
            mediainfo.actors = [{"name": actor.strip()} for actor in video.casts.split(',') if actor.strip()]

        # 标签/类型
        if video.genres:
            mediainfo.genres = [{"id": tag, "name": tag} for tag in video.genres.split(',') if tag.strip()]

        # 描述（使用发行商、制作商、系列等信息）
        description_parts = []
        if video.publisher:
            description_parts.append(f"发行商: {video.publisher}")
        if video.producer:
            description_parts.append(f"制作商: {video.producer}")
        if video.series:
            description_parts.append(f"系列: {video.series}")
        if description_parts:
            mediainfo.overview = " | ".join(description_parts)

        # 欧美系分类
        category = "成人/欧美系"
        mediainfo.set_category(category)
        logger.info(f"ByteMuse: 分类设置为 '{category}' (标题: {video.title})")
    ...
    def _recognize_with_theporndb_jav(self, title: str) -> Optional[MediaInfo]:
        """
        使用 ThePornDB JAV 识别媒体（同步版本）
        使用两步法 API 流程：
        1. 搜索 JAV 获取数字 ID
        2. 使用数字 ID 获取完整详情
        ...
        :return: 识别结果
        """
        logger.info(f"ThePornDB JAV: 正在识别 '{title}' ...")

        try:
            # 第一步：搜索 JAV
            results = self._theporndb_client.search_javs(title)
            if not results:
                logger.warning(f"ThePornDB JAV: '{title}' 未找到匹配结果")
                return None

            # 取第一个结果
            jav = results[0]
            logger.info(f"ThePornDB JAV: 搜索到结果 - {jav.get('title', '')} (ID: {jav.get('id')})")

            # 第二步：获取详情（使用数字 ID）
            detail = None
            if jav.get('id'):
                try:
                    detail = self._theporndb_client.get_jav_detail(str(jav['id']))
                    if detail:
                        logger.info(f"ThePornDB JAV: 获取详情成功 - UUID: {detail.get('id')}, 番号: {detail.get('external_id')}")
                except Exception as e:
                    logger.debug(f"ThePornDB JAV: 获取详情失败: {str(e)}")

            # 将搜索结果转换为 Pydantic 模型
            from .schema import ThePornDBJAV, ThePornDBJAVDetail, ThePornDBJAVBackground, ThePornDBJAVPerformer, ThePornDBJAVSite, ThePornDBTag

            # 构建 ThePornDBJAV 对象
            jav_bg = jav.get('background', {})
            jav_background = ThePornDBJAVBackground(
                full=jav_bg.get('full', ''),
                large=jav_bg.get('large', ''),
                medium=jav_bg.get('medium', ''),
                small=jav_bg.get('small', ''),
                thumb=jav_bg.get('thumb'),
                url=jav_bg.get('url', '')
            )

            jav_site_data = jav.get('site', {})
            jav_site = ThePornDBJAVSite(
                id=jav_site_data.get('id', 0),
                name=jav_site_data.get('name', ''),
                short_name=jav_site_data.get('short_name', ''),
                url=jav_site_data.get('url', ''),
                logo=jav_site_data.get('logo'),
                favicon=jav_site_data.get('favicon')
            )

            jav_performers = []
            for p in jav.get('performers', []):
                jav_performers.append(ThePornDBJAVPerformer(
                    id=str(p.get('id', '')),
                    name=p.get('name', ''),
                    gender=p.get('gender'),
                    link=p.get('link', ''),
            ...
            jav_model = ThePornDBJAV(
                id=str(jav.get('id', '')),
                _id=jav.get('id', 0),
                title=jav.get('title', ''),
                type=jav.get('type', 'JAV'),
                slug=jav.get('slug', ''),
                external_id=jav.get('external_id', ''),
                date=jav.get('date'),
                duration=jav.get('duration'),
                background=jav_background,
                poster=jav.get('poster', ''),
                link=jav.get('link', ''),
                performers=jav_performers,
                site=jav_site
            )

            # 如果有详情，转换为 Pydantic 模型
            detail_model = None
            if detail:
                detail_bg = detail.get('background', {})
                detail_background = ThePornDBJAVBackground(
                    full=detail_bg.get('full', ''),
                    large=detail_bg.get('large', ''),
                    medium=detail_bg.get('medium', ''),
                    small=detail_bg.get('small', ''),
                    thumb=detail_bg.get('thumb'),
                    url=detail_bg.get('url', '')
                )

                detail_site_data = detail.get('site', {})
                detail_site = ThePornDBJAVSite(
                    id=detail_site_data.get('id', 0),
                    name=detail_site_data.get('name', ''),
                    short_name=detail_site_data.get('short_name', ''),
                    url=detail_site_data.get('url', ''),
                    logo=detail_site_data.get('logo'),
                    favicon=detail_site_data.get('favicon')
                )

                detail_performers = []
                for p in detail.get('performers', []):
                    detail_performers.append(ThePornDBJAVPerformer(
                        id=str(p.get('id', '')),
                        name=p.get('name', ''),
                        gender=p.get('gender'),
                        link=p.get('link', ''),
                ...
                for t in detail.get('tags', []):
                    detail_tags.append(ThePornDBTag(
                        id=t.get('id'),
                        name=t.get('name', '')
                    ))

                detail_model = ThePornDBJAVDetail(
                    id=detail.get('id', ''),
                    title=detail.get('title', ''),
                    type=detail.get('type', 'JAV'),
            ...
            # 转换为 MediaInfo
            mediainfo = self._convert_theporndb_jav_to_mediainfo(jav_model, detail_model)

            self._add_log(title, f"{mediainfo.title} ({mediainfo.year})", "success",
                          "来源: ThePornDB JAV", category=self._build_category(self.SUBCATEGORY_JAV))
            logger.info(f"ThePornDB JAV: 识别成功 - {title} -> {mediainfo.title} ({mediainfo.year})")
    ...
    async def _async_recognize_with_theporndb_jav(self, title: str) -> Optional[MediaInfo]:
        """
        异步使用 ThePornDB JAV 识别媒体

        :param title: 搜索标题/番号
        :return: 识别结果
        """
        logger.info(f"ThePornDB JAV: 正在异步识别 '{title}' ...")

        try:
            # 异步搜索 JAV
            results = await self._theporndb_client.async_search_javs(title)
            if not results:
                logger.warning(f"ThePornDB JAV: '{title}' 未找到匹配结果")
                return None
            ...
            if jav.id:
                try:
                    detail = await self._theporndb_client.async_get_jav_detail(jav.id)
                except Exception as e:
                    logger.debug(f"ThePornDB JAV: 获取详情失败: {str(e)}")

            # 转换为 MediaInfo
            mediainfo = self._convert_theporndb_jav_to_mediainfo(jav, detail)
    ...
    def _recognize_with_bytemuse(self, title: str) -> Optional[MediaInfo]:
        ...
        try:
            ...
            if not results:
                logger.warning(f"ByteMuse: '{title}' 未找到匹配结果")
                return None
            ...
            # 转换为 MediaInfo
            mediainfo = self._convert_bytemuse_to_mediainfo(video)

            category = self._build_category(self.SUBCATEGORY_WESTERN)
            self._add_log(title, f"{mediainfo.title} ({mediainfo.year})", "success",
                          "来源: ByteMuse", category=category)
            logger.info(f"ByteMuse: 识别成功 - {title} -> {mediainfo.title} ({mediainfo.year})")
    ...
    def recognize_media(self, meta: MetaBase = None,
        ...
        # Step 4: 优先使用 Metatube API（核心识别API）
        logger.info(f"Metatube: 优先使用核心 Metatube API 识别 '{number}'")
        try:
            # 搜索
            ...
            if results:
                ...
                # 根据分类设置
                category = self._build_category(detected_category)
                self._add_log(number, mediainfo.title, "success",
                              f"来源: Metatube ({movie.provider})", category=category)
                logger.info(f"Metatube: 核心API识别成功 - {number} -> {mediainfo.title} (分类: {category})")

                return mediainfo

        except Exception as e:
            logger.debug(f"Metatube: 核心API识别失败: {str(e)}")

        # Metatube 识别失败，尝试补充 API
        logger.info(f"Metatube: 核心API识别失败，尝试补充识别方案")

        # 检查是否为日本番号或欧美系内容
        is_jav_number = self._is_jav_number(number)

        # 补充方案 1: ThePornDB（针对特定分类）
        if self._theporndb_enabled and self._theporndb_api_token:
            if is_jav_number:
                # 日本番号，使用 JAV API
                logger.info(f"Metatube: 尝试 ThePornDB JAV API 识别 '{number}'")
                result = self._recognize_with_theporndb_jav(number)
                if result:
                    return result
            elif detected_category == self.SUBCATEGORY_WESTERN:
                # 欧美系内容，使用 Scene API
                logger.info(f"Metatube: 尝试 ThePornDB Scenes API 识别 '{title}'")
        ...
        # 补充方案 2: ByteMuse
        if self._bytemuse_enabled and self._bytemuse_api_token:
            logger.info(f"Metatube: 尝试 ByteMuse API 识别 '{number}'")
            result = self._recognize_with_bytemuse(number)
            if result:
                return result

        # 所有 API 识别失败，失败处理
        return self._handle_recognition_failure(number, title, "所有识别API均失败")
