...
from .api import MubeyClashRPApi, apis
...
from .services import MubeyClashRPService


class MubeyClashRP(_PluginBase):
    # 插件名称
    plugin_name = "Mubey Clash RP"
    # 插件描述
    ...
    # 插件配置项ID前缀
    plugin_config_prefix = "mubeyclashrp_"
    # 加载顺序
    plugin_order = 99
    # 可使用的用户级别
    auth_level = 1
    ...
    # Runtime variables
    services: MubeyClashRPService | None = None
    api: MubeyClashRPApi | None = None
    ...
    def _initialize_plugin(self):
        ...
        self.services = MubeyClashRPService(self.__class__.__name__, self.state, self.scheduler)
        self.api = MubeyClashRPApi(self.services, self.state.config)
