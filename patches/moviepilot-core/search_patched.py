        ...
        if mediaid.startswith("tmdb:"):
            ...
            if settings.RECOGNIZE_SOURCE == "themoviedb":
                ...
                if doubaninfo:
                    ...
                    yield {"type": "error", "success": False, "message": "未识别到豆瓣媒体信息"}
                    return
        elif mediaid.startswith("metatube_search:") or mediaid.startswith("metatube:"):
            # 成人番号搜索：提取纯番号，用番号搜索站点资源
            code = mediaid.split(":", 1)[1]
            torrents = search_chain.async_search_by_title_stream(
                title=code,
                sites=site_list,
                search_type="adult"
            )
...
async def search_by_title_stream(request: Request,
    ...
    if keyword and (keyword.startswith("metatube_search:") or keyword.startswith("metatube:")):
        search_keyword = keyword.split(":", 1)[1]
