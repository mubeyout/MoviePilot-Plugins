...
class SearchChain(ChainBase):
    ...
    _adult_keywords = [
        ...
        'KATU', 'ABS', 'SVDVD', 'DAP', 'SKY', 'HUN', 'FST', 'GS',
        ...
        'HEYZO', '10MUSUME', 'CARIB', 'GF', 'PACO', 'heyzo',
        'FC2', 'SR', 'SV', 'AD', 'BGM', 'RBD',
        'DL', 'TG',
    ]
    _adult_cn_keywords = ['无码', '有码', '步兵', '骑兵', '中文字幕', '成人', '色情', 'AV',
                         '一本道', '东京热', '热码', '无修', '素人', '流出', '盗摄', '偷拍']

    def _is_adult_keyword(self, keyword: str) -> bool:
        """
        自动识别关键词是否为成人内容
        """
        if not keyword:
        ...
        for prefix in self._adult_keywords:
            if kw_upper.startswith(prefix) or f"{prefix}-" in kw_upper:
                return True
        for kw in self._adult_cn_keywords:
            if kw in keyword:
                return True
        return False
    ...
    def search_by_title(self, title: str, page: Optional[int] = 0,
        ...
        :param sites: 站点ID列表
        :param cache_local: 是否缓存到本地
        :param search_type: 搜索类型 normal/adult/all/auto，默认 auto（自动识别）
        """
        if title:
        ...
        if search_type is None:
            search_type = 'auto'
        # 搜索
        torrents = self.__search_all_sites(keyword=title, sites=sites, page=page,
    ...
    async def async_search_by_title(self, title: str, page: Optional[int] = 0,
        ...
        :param cache_local: 是否缓存到本地
        """
        # 处理 bytemuse: 前缀（ByteMuse 番号直接搜索）
        original_title = title
        if title and title.startswith("bytemuse:"):
            title = title.replace("bytemuse:", "", 1)
            logger.info(f'ByteMuse 番号搜索: {original_title} -> {title}')
        if title:
            logger.info(f'开始搜索资源，关键词：{title} ...')
        else:
            logger.info(f'开始浏览资源，站点：{sites} ...')
    ...
    async def async_search_by_title_stream(self, title: str, page: Optional[int] = 0,
        ...
        search_type: adult/normal/all/auto，默认 auto
        """
        if title:
    ...
    def __search_all_sites(self, keyword: str,
        ...
        :param search_type:  搜索类型 normal/adult/all/auto
        :reutrn: 资源列表
        """
        # 自动识别成人内容关键词
        if search_type == 'auto' and keyword:
            search_type = 'adult' if self._is_adult_keyword(keyword) else 'normal'
        # 未开启的站点不搜索
        indexer_sites = []
