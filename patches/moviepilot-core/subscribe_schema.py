...
class Subscribe(BaseModel):
    ...
    @field_validator("tmdbid", mode="before")
    @classmethod
    def _coerce_tmdbid(cls, v):
        """Accept string tmdbid (e.g. metatube numbers) and convert non-int to None."""
        if v is None:
            return None
        if isinstance(v, int):
            return v
    ...
    def _coerce_year(cls, v):
        """Accept int year and convert to string."""
        if v is None:
            return None
        return str(v)
...
class SubscribeShare(BaseModel):
    ...
    @field_validator("tmdbid", mode="before")
    @classmethod
    def _coerce_tmdbid_share(cls, v):
        """Accept string tmdbid and convert non-int to None."""
        if v is None:
            return None
        if isinstance(v, int):
            return v
