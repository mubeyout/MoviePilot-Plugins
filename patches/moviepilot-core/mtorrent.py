...
class MTorrentSpider:
    ...
    # 成人分类（Jackett Torznab Category 体系）
    # 标准成人: 6000=XXX, 6010=DVD, 6060=ImageSet, 6080=SD
    # MTeam 自定义成人分类
    _adult_category = [
        # 标准成人父分类
        '6000', '6010', '6060', '6080',
        # AV 有码 HD/Blu-Ray/SD/DVD
        '100410', '100431', '100437', '100424',
        # AV 无码 HD/Blu-Ray/SD/DVD
        '100429', '100432', '100430', '100426',
        # AV 网站 0Day
        '100436',
        # H-Anime/Comic/Game
        '100412', '100413', '100411',
        # IV 写真
        '100425', '100433',
        # Gay HD
        '100440',
    ...
    def __get_params(self, keyword: str, mtype: MediaType = None, page: Optional[int] = 0) -> list:
        """
        获取请求参数，返回普通搜索和成人搜索两套参数
        参照 Jackett Category 过滤方式，同时搜索普通内容和成人内容
        """
        # 普通搜索分类
        if not mtype:
            normal_categories = self._movie_category + self._tv_category
        elif mtype == MediaType.TV:
            normal_categories = self._tv_category
        else:
            normal_categories = self._movie_category
        ...
        # 成人搜索参数（mode=adult 包含成人内容）
        adult = {
            "keyword": keyword,
            "categories": [],
            "pageNumber": int(page) + 1,
            "pageSize": min(self._size, 30),
            "visible": 1,
            "mode": "adult"
        }
        return [normal, adult]
    ...
    def __parse_result(self, results: List[dict]):
        ...
        for result in results:
            ...
            if category_value in self._tv_category \
                    ...
                    and category_value not in self._adult_category:
                category = MediaType.TV.value
            elif category_value in self._movie_category:
                category = MediaType.MOVIE.value
            elif category_value in self._adult_category:
                # 成人内容标记为电影（PRDoc 要求混排）
                category = MediaType.MOVIE.value
            else:
    ...
    def search(self, keyword: str, mtype: MediaType = None, page: Optional[int] = 0) -> Tuple[bool, List[dict]]:
        """
        搜索（同时搜索普通内容和成人内容，参照 Jackett Category 过滤方式）
        """
        # 检查ApiKey
        if not self._apikey:
            return True, []
        ...
        # 获取两套参数（普通 + 成人）
        params_list = self.__get_params(keyword, mtype, page)
        all_results = []
        ...
        for params in params_list:
            ...
            if res and res.status_code == 200:
                ...
                logger.warn(f"{self._name} 搜索失败(mode={params.get('mode','normal')})，错误码：{res.status_code}")
            else:
                logger.warn(f"{self._name} 搜索失败(mode={params.get('mode','normal')})，无法连接 {self._domain}")

        return False, self.__parse_result(all_results)

    async def async_search(self, keyword: str, mtype: MediaType = None, page: Optional[int] = 0) -> Tuple[bool, List[dict]]:
        """
        异步搜索（同时搜索普通内容和成人内容，参照 Jackett Category 过滤方式）
        """
        # 检查ApiKey
        if not self._apikey:
            return True, []
        ...
        async def _search_one(params: dict) -> List[dict]:
            try:
                ...
                if res and res.status_code == 200:
                    return res.json().get('data', {}).get("data") or []
                elif res is not None:
                    logger.warn(f"{self._name} 异步搜索失败(mode={params.get('mode','normal')})，错误码：{res.status_code}")
                else:
                    logger.warn(f"{self._name} 异步搜索失败(mode={params.get('mode','normal')})，无法连接 {self._domain}")
            except Exception as e:
                logger.warn(f"{self._name} 异步搜索异常(mode={params.get('mode','normal')}): {e}")
            return []

        # 并发搜索普通 + 成人
        tasks = [_search_one(p) for p in params_list]
